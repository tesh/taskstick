...
**Status:** Deployed. Confirmed EventKit has no better/official alternative (researched: Apple provides no Reminders REST API — every third-party "API" found is itself an EventKit wrapper). Still awaiting Tesh's live retest with the permission fixes in place.
