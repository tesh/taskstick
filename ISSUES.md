...
**Also resolved the same day:** Google approved the `pps-taskstick` OAuth verification on 2026-08-22 and the consent screen is now in Production. That closes off the other possible cause of this symptom — Testing-mode apps have Google's own hard 7-day refresh-token cap, independent of anything in the session fix above. With Production status, that cap no longer applies at all.
**Files:** `config.php`
**Resolved:** 2026-08-22
