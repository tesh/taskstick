...
### BUG-020 · fixed · 2026-09-06
**Title:** Two CSS declarations were silently invalid, so a hover ring and the subtask rail never rendered in any browser
**Found by:** An independent review pass, as an aside while reviewing ENH-053/054/055 — not user-reported.
**Root cause:** Both `.list-card:hover`'s `box-shadow` and `.task-item.subtask`'s `border-left` appended an alpha suffix directly to a custom property: `var(--accent)55`. `var()` substitutes a whole token, so this produced `2px solid #6366f1 55` — an invalid value, which per CSS error handling causes the *entire declaration* to be dropped. The hex-plus-alpha trick only works on a literal hex, never through a variable, and it fails silently: no console error, the rule just never applies. The subtask "accent-tinted rail" that a code comment describes has therefore never once been visible, and neither has the list-card hover outline.
**Fix:** Both now use `color-mix(in srgb, var(--accent) N%, transparent)`, which is variable-safe and also works for the Purple Pill theme, whose `--accent` is an `oklch()` value that the hex trick could never have supported.
**Files:** `index.html`
**Resolved:** 2026-09-06
...
**Also resolved the same day:** Google approved the OAuth verification on 2026-08-22 and the consent screen is now in Production. That closes off the other possible cause of this symptom — Testing-mode apps have Google's own hard 7-day refresh-token cap, independent of anything in the session fix above. With Production status, that cap no longer applies at all.
**Files:** `config.php`
**Resolved:** 2026-08-22
...
### ENH-055 · complete · 2026-09-06
**Title:** Snoozed divider: drop the "SNOOZED" label, add a genuinely visible rule
**Requested by:** Tesh ("remove the title SNOOZED for the snoozed tasks and just use the visual zzz and the number of snoozed tasks. Also let's have a clear horizontal rule that delineates active and snoozed tasks")
**Root cause of the invisible line:** the divider's separator was `border-top: 1px dashed var(--rule-color)`, and `--rule-color` is deliberately `transparent` in the Notebook theme (the default — that theme draws its own ruled-paper lines instead). So in the theme Tesh actually uses, there was no line at all.
**Fix:** The divider is now just 💤, the count, and the chevron — the `title` attribute carries the wording ("Hide 3 snoozed tasks", correctly singular/plural) now that the visible label is gone. The rule is drawn by a `.snooze-divider::before` pseudo-element using `background: currentColor` at 0.5 opacity rather than a themed variable, so it renders in all six themes regardless of how each defines its colors, and tints toward the accent on hover along with the rest of the row. Also gave the divider more vertical breathing room so it reads as a section break rather than another row.
**Files:** `index.html`
**Resolved:** 2026-09-06

### ENH-054 · complete · 2026-09-06
**Title:** Use the existing notes button as the has-notes indicator instead of a second icon
**Requested by:** Tesh ("rather than having a separate notes icon displayed for a task with a note, could we just use the existing note icon in the row of icons and keep that visible when a task has a note") — with a screenshot showing both icons at once
**Design decision:** ENH-051 had added a separate 📝 next to the task title because the original button lives in `.task-actions-row`, which is `max-height: 0; opacity: 0` until hover. That worked but showed two note icons on the same task. Now the row itself is revealed for a `.has-notes` task, and on pointer devices only the notes button is visible until hover (`@media (min-width: 641px)` + `.task-item.has-notes:not(:hover) .task-actions-row > *:not(.notes-btn)`) — otherwise every annotated task would sit there displaying a full toolbar. The media query matters: `:not(:hover)` never resolves false on touch, so without it the rule would have hidden every other button permanently on mobile, where the row is always shown by design.
**Files:** `index.html`
**Resolved:** 2026-09-06

### ENH-053 · complete · 2026-09-06
**Title:** Remove the Follow-up feature entirely now that Snooze replaces it
**Requested by:** Tesh ("Now that we have the Snoozed feature, let's remove the Follow-up list")
**Fix:** Removed `moveBackFromFollowUp()`, `getOriginListId()`, the legacy ◑ `.partial-btn` (markup and CSS), and all `isFollowUp`/`originListId` plumbing from `renderTaskItem()` and `renderPriorityTaskItem()`; `renderSnoozeButtons()` collapsed to `renderSnoozeButton(listId, task, {done, isSnoozed})`. `stripOriginMarker()` and `ORIGIN_RE` were **deliberately kept**: a task that was deferred under the old feature and never moved back still carries a `[taskstick-origin:…]` marker in its notes, and without the strip that raw text would start showing in the notes panel. A list named "Follow-up" is now just an ordinary list — renamable, unpinned, included in Priority, no special styling.
**Not done (deliberately):** the user's actual "Follow-up" task list in Google Tasks was **not** deleted. Deleting a real list with real tasks in it is destructive and irreversible, isn't required by the code change, and there's no way for this session to see whether it still holds anything. It can be deleted from the app with the 🗑 button on the list card.
**Files:** `index.html`
**Resolved:** 2026-09-06
...
### ENH-051 · complete (superseded same day by ENH-054) · 2026-09-06
**Title:** Always-visible indicator on tasks that have notes
**Superseded:** The separate icon described below was replaced hours later by ENH-054, which keeps the *existing* actions-row button visible instead. Kept here for the reasoning, which ENH-054 builds on.
...
**Fix:** New `task_dates_db.php` (MySQL table `task_created_dates`, task_id + user_email + first_seen_at) — auto-creates on first use, same pattern as `feedback_db.php`. `api/tasks.php`'s GET backfills+returns dates for every task in a list; POST (create) records the exact moment. Client-side: `state.taskCreatedDates` (task_id → ISO date) merged in during `loadAll()`; colored green→amber→red by linear interpolation between three fixed stops (not a flat green-red blend, which reads as muddy brown at the midpoint) based on age ÷ threshold. New Settings toggle "Task age indicator" with the "days until aged" number input folded directly into its own description sentence rather than a separate row, so the two visually read as one connected setting; both ride the existing generic `state.settings` persistence — no backend changes needed for the settings themselves. Also fixed in passing: `db.php`'s MySQL connection never forced a session timezone, so `NOW()` wasn't safely assumable as UTC — added `SET time_zone = '+00:00'` on connect, which this feature's date math depends on and every other timestamp in the app (e.g. feedback's `created_at`) now benefits from too.
**Revised same day:** First version added a separate colored dot next to each task, in its own settings row from the threshold input. Tesh asked for the existing task-check circle's own ring to be recolored instead (no extra dot), and for the threshold input to visually read as tied to the toggle rather than a second unrelated setting. `renderAgeDot()` (returned a standalone `<span>`) became `ageIndicatorAttrs()` (returns a class/style/title spliced into the check-circle span itself via a new `.has-age` class keyed off a `--age-color` custom property); the two settings rows merged into one, with the number input inline inside the description text. Caught and fixed one regression before shipping: unconditionally adding a `title` attribute to the circle — even an empty one when age tracking doesn't apply — would have suppressed the parent label's existing "Complete task" tooltip rather than falling through to it, since a present-but-empty `title` doesn't fall back to an ancestor's; the attribute is now only emitted when there's an actual age label to show.
**Verification:** Full integration test against a real local MariaDB instance (not just PHP's syntax check) — table auto-creation, backfill-without-overwrite (INSERT IGNORE), per-user isolation on the same task ID, empty-input handling, and a live check that MySQL's `NOW()` matches PHP's UTC clock within seconds, confirming the timezone fix actually works rather than assuming it. Frontend helpers exercised directly in a browser: color stops verified exact at 0/50/100% of the threshold, clamping past 100% confirmed, completed tasks and tasks with no recorded date both correctly render with no age color, the settings toggle/number input verified to update state end-to-end, and the merged circle-coloring + settings-row changes re-verified visually (including dark theme) with fake task data before deploying.
...
**Fix:** New `feedback_db.php` — PDO/MySQL layer, auto-creates the `feedback` table (`id, type, description, submitter_email, submitter_name, status, created_at`) on first use. New `api/feedback.php` (`POST {type, description}`, authenticated users only). `api/admin.php` exposes `GET ?resource=feedback` and `POST {action:'update_feedback_status', id, status}`. Frontend: "Send Feedback" item in the profile dropdown opens a modal (type select + description textarea); Admin → Feedback lists all submissions with a status dropdown (New/Reviewed/Resolved). `config.php` now holds the IONOS MySQL credentials (host/db name redacted); verified end-to-end with a temporary connectivity check (uploaded, tested `feedbackDb()` successfully creates the table, then removed) before wiring it in for real.
