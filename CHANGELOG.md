...
- **Added:** Task age indicator — each task's own circle is colored green when it's new, fading through amber to red the longer it's been sitting, with a Settings toggle and a configurable "days until aged" threshold folded right into that same setting's description. Google Tasks has no creation-date field, so this is tracked in a small new database table the app keeps itself (ENH-050).
...
- **Added (beta):** Apple Reminders sync — a native macOS menu-bar helper app (`mac-helper/`, built on EventKit) pushes tasks to Reminders on your Mac. Enable it in Settings to get a setup token, connect the helper app with it, then pick which lists sync; a matching Reminders list is created automatically if one doesn't already exist. Google Tasks stays the source of truth — completing a reminder marks the task done too, but title/notes/due-date edits only flow one way, out from Google Tasks (ENH-047).
