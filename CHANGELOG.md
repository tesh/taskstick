...
- **Rebuilt:** Apple Reminders sync's CalDAV approach (ENH-042/043/044) turned out to be a dead end — Apple removed Reminders from iCloud's CalDAV protocol in 2019, so pushes reported success but never actually reached Reminders on any device. Confirmed via a server-side round-trip check and cross-referenced against BusyCal/DAVx5/Apple forum reports, which all independently document the same limitation. Replaced with a native macOS menu-bar helper app (`mac-helper/`, EventKit-based — the only API that still has real Reminders access) polling a new lightweight export endpoint. Settings now issues a setup token instead of storing an Apple ID + app-specific password. The old CalDAV code is preserved in `archive/apple-caldav-attempt-2026-08-19/` (ENH-047).
